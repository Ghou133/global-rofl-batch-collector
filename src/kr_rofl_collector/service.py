...
class CollectorService:
    ...
    def _patch(self) -> tuple[str, str]:
        resolver = PatchResolver(timeout=self.config.request_timeout)
        try:
    ...
    def _probe_locked(self) -> dict[str, Any]:
        ...
        dataset_id = self.db.dataset("KR", patch, realm_version)
        ...
        try:
            ...
            match_ids = api.match_ids(str(top["puuid"]), count=1)
            if not match_ids:
                raise CollectorError(
                    "NO_RECENT_MATCH", "Top Challenger has no recent queue 420 match"
                )
            raw_match = api.match(match_ids[0])
            record = parse_match(raw_match, self.db.player_tiers(dataset_id))
            eligible = DiscoveryService.eligible(record, patch)
    ...
    def _run_locked(self, target: int) -> dict[str, Any]:
        ...
        api: RiotApi | None = None
        acquirer: ReplayBackendAcquirer | None = None
        summary: dict[str, Any] = {"patch": patch, "target": target, "run_id": run_id}
        try:
            verified_before = self.db.verified_count(dataset_id)
            summary["verified_before"] = verified_before
            if verified_before >= target:
                path = write_manifest(self.db, dataset_id, self.config.data_dir, patch)
                summary.update(
                    {
            ...
            candidates = self.db.candidates(dataset_id)
            if not candidates:
                raise CollectorError(
                    "NO_ELIGIBLE_MATCHES",
                    "No completed current-patch KR queue 420 matches were found",
                )
            ...
            for candidate in self.db.candidates(dataset_id):
                ...
                if downloaded_this_run % 10 == 0:
                    write_manifest(self.db, dataset_id, self.config.data_dir, patch)
    ...
    def _write_run_report(self, run_id: int, patch: str, summary: dict[str, Any]) -> Path:
        path = self.config.data_dir / "KR" / patch / "reports" / f"run_{run_id}.json"
        _atomic_json(path, summary)
        return path

    def status(self, *, verify_files: bool = False) -> dict[str, Any]:
        dataset = self.db.latest_dataset("KR")
        if dataset is None:
            return {
                "initialized": True,
                "current_patch": "UNKNOWN",
                "message": "No dataset has been created. Run collector probe first.",
            }
        ...
        result: dict[str, Any] = {
            ...
            **stats,
            ...
            "rofl_root": str(self.config.data_dir / "KR" / dataset["patch_key"] / "builds"),
            "manifest": str(manifest_path(self.config.data_dir, dataset["patch_key"])),
            "logs": str(self.config.logs_dir),
            "reports": str(
                self.config.data_dir / "KR" / dataset["patch_key"] / "reports"
            ),
        }
        if verify_files:
            errors: list[str] = []
        ...
        with CollectorLock(self.config.data_dir / ".collector.lock"):
            ...
            if dataset is None:
                raise ConfigurationError(
                    "DATASET_MISSING", "No KR dataset exists; run collector probe first"
                )
